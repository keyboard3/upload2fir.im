...
if(strings.indexOf('app_name') > 0){
  appName = strings.match(/string name="app_name".*>(\S+)</)[1];
}
if (!appName) {
  ...
  let regexSuffix = new RegExp("^\\s+buildTypes\\s+{[\\s\\S\\n]+?\\s+"+activeApkType+"[\\s\\S\\n]+?versionNameSuffix\\s'(.+)'",'gim');
  let matchSuffix = regexSuffix.exec(buildConfig);
  let versionNameSuffix = matchSuffix && matchSuffix.length > 0 ? matchSuffix[1]: '';
  appName += versionNameSuffix;
}
